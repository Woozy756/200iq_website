...
function getLocalizedPath(locale: Lang, pattern: PagePattern): string {
...
function renderAlternates(pattern: PagePattern, siteOrigin: string): string {
	return locales
		.map((locale) => {
			const href = toAbsolute(getLocalizedPath(locale, pattern), siteOrigin);
...
function renderUrlEntries(siteOrigin: string): string {
	return pagePatterns
		.flatMap((pattern) =>
			locales.map((locale) => {
				const loc = toAbsolute(getLocalizedPath(locale, pattern), siteOrigin);
